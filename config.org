...
:VISIBILITY: all
:END:
- [[#property-header-argsemacs-lisp-tangle-initel-mkdirp-yes][(#+PROPERTY: header-args:emacs-lisp :tangle ./init.el :mkdirp yes)]]
- [[#about][About]]
  - [[#nix][Nix]]
...
- [[#org-mode][Org Mode]]
  ...
  - [[#planning-with-time-blocks][Planning with Time Blocks]]
  - [[#caldav][CalDAV]]
- [[#matrix][Matrix]]
...
This config block is to enable installing packages with emacs
#+begin_src emacs-lisp
...
This config block is for when all the emacs packages are installed with nix
#+begin_src emacs-lisp
...
Define a major-mode specific to asdf files which hides narrows the buffer to just the yaml document.
#+begin_src emacs-lisp
...
Add support for loading very large files
...
This is cribbed from [[https://mentat.za.net/blog/2026/01/27/org-mode-time-blocks/][Stefan's blog]].

This requires two capture templates (see [[file:/home/stuart/Git/new.emacs.d/config.org::*Capture][Capture]]) which look like this:
...
The week template:
#+BEGIN_SRC org
%(org-capture-put :custom-timestamp (org-read-date))

\* %(format-time-string "\%Y-W\%W" (date-to-time (org-capture-get :custom-timestamp)))
#+END_SRC
...
This helper is a way of inserting links to other org headings quickly, so I can link to existing tasks from the plan
#+begin_src emacs-lisp
